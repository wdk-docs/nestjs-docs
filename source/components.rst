...
``@Injectable()`` decorator preceding their class declaration.

   info **Hint** Since Nest enables the possibility to design and
...
Let’s start by creating a simple ``CatsService``. This service will be
...
provider. Thus, we decorate the class with ``@Injectable()``.
...
``Cat`` interface, which probably looks something like this:
...
Now that we have a service class to retrieve cats, let’s use it inside
the ``CatsController``:
...
Nest is built around the strong design pattern commonly known as
...
In Nest, thanks to TypeScript capabilities, it’s extremely easy to
...
controller’s constructor (or assigned to the indicated property):
...
Providers normally have a lifetime (‘scope’) synchronized with the
...
techniques `here </fundamentals/injection-scopes>`__.
...
`here </fundamentals/dependency-injection>`__.

Optional providers
^^^^^^^^^^^^^^^^^^

Occasionally, you might have dependencies which do not necessarily have
...
configuration provider wouldn’t lead to errors.
...
Note, in the example above we are using a custom provider, which is the
...
their associated tokens `here </fundamentals/custom-providers>`__.

Property-based injection
^^^^^^^^^^^^^^^^^^^^^^^^

The technique we’ve used so far is called constuctor-based injection, as
...
the ``@Inject()`` decorator at the property level.
...
..
   ...
   warning **Warning** If your class doesn’t extend another provider,
   you should always prefer using **constructor-based** injection.

Provider registration
^^^^^^^^^^^^^^^^^^^^^

Now that we have defined a provider (``CatsService``), and we have a
...
the ``providers`` array of the ``@Module()`` decorator.
...
Nest will now be able to resolve the dependencies of the
