...
There are several ways of mounting the Nest application. You can create
...
even build a **CLI** on top of it.

Getting started
^^^^^^^^^^^^^^^

In order to create a Nest application context, we use the following
syntax:
...
Afterward, Nest allows you to pick any instance registered within Nest
...
want to call from within CRON job.
...
And that’s it. To grab ``TasksService`` instance we had to use ``get()``
...
instance from the selected context.
...
..
   ...
   info **Hint** In non-strict mode, the root module is selected by
