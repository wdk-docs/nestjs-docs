...
The applications used to run in different **environments**. Depending on
...
since this approach is very convenient.

But when we use a ``process`` global object, it’s difficult to keep our
...
``ConfigService`` with loaded configuration variables.

Installation
^^^^^^^^^^^^

Certain platforms automatically attach our environment variables to the
...
use a `dotenv <https://github.com/motdotla/dotenv>`_ package.
...
This class takes a single argument, a ``filePath``, which is a path to
...
The ``ConfigModule`` registers a ``ConfigService`` and exports it as
...
value based on a passed key. Sample ``.env`` file could look like below:
...
To access **environment variables** from our ``ConfigService`` we need
to inject it. Therefore we firstly need to import the module.
...
Afterward, you can inject it using an injection token. By default, the
token is equal to the class name (in our example ``ConfigService``).
...
..
   ...
   info **Hint** Instead of importing ``ConfigModule`` in all your
   modules, you can also declare ``ConfigModule`` as a global module.

Advanced configuration
^^^^^^^^^^^^^^^^^^^^^^

We just implemented a basic ``ConfigService``. However, this approach
has a couple of disadvantages, which we’ll address now:

-  missing names & types for the environment variables (no IntelliSense)
-  a lack of **validation** of the provided ``.env`` file
-  the env file provides booleans as string (``'true'``), and thus have
   to cast them to a ``boolean`` every time

Validation
^^^^^^^^^^

We’ll start with the validation of the provided environment variables.
...
schema and validate JavaScript objects against it.
...
Since we set default values for ``NODE_ENV`` and ``PORT`` the validation
...
the env strings into the right type.
