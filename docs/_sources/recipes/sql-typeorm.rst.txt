...
'''''''''''''''''''''''''''''''''''''''
   ...
   **Warning** In this article, you’ll learn how to create a
   ...
   learn more, see `here </techniques/sql>`__.

`TypeORM <https://github.com/typeorm/typeorm>`_ is definitely the most
...
..
   ...
   warning **Notice** In the real-world applications you should avoid
   **magic strings**. Both ``PHOTO_REPOSITORY`` and
   ``DATABASE_CONNECTION`` should be kept in the separated
   ``constants.ts`` file.

Now we can inject the ``Repository<Photo>`` to the ``PhotoService``
...
..
   ...
   warning **Hint** Do not forget to import the ``PhotoModule`` into the
